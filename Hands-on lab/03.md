...
1.  Complete the **Create storage account** form using the following details, then select **Review + create**:
    ...
    - **Location**: Select the same location which you choose while creating **Recovery Services Vault**.
      ...
      ![Fields in the Create storage account blade are set to the previously defined settings.](images/Hands-onlabstep-bystep-Businesscontinuityanddisasterrecoveryimages/media/image407.png "Create storage account blade")
    
1.  Repeat the above steps to create another storage account but this time select **Premium** under **Performance** tier.

    ![Fields in the Create storage account blade are set to the previously defined settings.](images/Hands-onlabstep-bystep-Businesscontinuityanddisasterrecoveryimages/media/image408.png "Create storage account blade")

### Task 2: Configure on-premises to Azure IaaS failover
...
50. You will be failing the **OnPremVM** into the **Secondary** site that was deployed with your IaaS installation, so you already have a Virtual Network created. Select **Next** on the Target blade to continue.
...
56. On the Source blade select: **Source location**: **OnPremHyperVSite** and select **OK**.
...
### Task 3: Configure IaaS SQL Always On availability groups for region to region failover
...
5. Complete the **Create storage account** form using the following details, then select **Next**:
...
6.  On the **Advanced** tab select **Minimum TLS version** as **Version 1.0**, select **Review + Create**.
...
### Task 4: Configure IaaS for region to region failover
...
### Task 5: Configure PaaS for region to region failover
